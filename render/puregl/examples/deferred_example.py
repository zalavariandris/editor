...
class Pointlight:
    ...
    def views(self):
        ...
        shadowTransforms.append(
            glm.lookAt(pointlight.position, pointlight.position + glm.vec3(1, 0, 0), glm.vec3(0, -1, 0)))
        shadowTransforms.append(
            glm.lookAt(pointlight.position, pointlight.position + glm.vec3(-1, 0, 0), glm.vec3(0, -1, 0)))
        shadowTransforms.append(
            glm.lookAt(pointlight.position, pointlight.position + glm.vec3(0, 1, 0), glm.vec3(0, 0, 1)))
        shadowTransforms.append(
            glm.lookAt(pointlight.position, pointlight.position + glm.vec3(0, -1, 0), glm.vec3(0, 0, -1)))
        shadowTransforms.append(
            glm.lookAt(pointlight.position, pointlight.position + glm.vec3(0, 0, 1), glm.vec3(0, -1, 0)))
        shadowTransforms.append(
            glm.lookAt(pointlight.position, pointlight.position + glm.vec3(0, 0, -1), glm.vec3(0, -1, 0)))
...
with window:
    ...
    glTexImage2D(
        GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, spotlight_shadowsize[0], spotlight_shadowsize[1], 0, GL_DEPTH_COMPONENT,
        GL_FLOAT, None
    )
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST)
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST)
